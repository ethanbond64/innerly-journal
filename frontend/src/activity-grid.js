...
// A day is as negative as its worst entry: one negative entry makes the day
// negative, one positive entry (with nothing negative) makes it positive, and a
// day of nothing but neutral entries stays neutral.
const daySentiment = (day) => day.negative > 0 ? 'negative' : (day.positive > 0 ? 'positive' : 'neutral');

// The weeks of one calendar year, oldest first, from the week January 1st falls
...
export const buildYearWeeks = (rows, year, today = new Date()) => {
    ...
    for (let cursor = startOfWeek(opening); cursor <= closing; cursor = addDays(cursor, 7)) {
        ...
        for (let weekday = 0; weekday < 7; weekday++) {
            ...
            // An undrawn day carries no totals, or the December shared with the
            // next year's grid would be counted twice when both are on the page.
...
// The window a year's entries are fetched for: January 1st up to December 31st,
// or up to today for the year in progress.
export const yearWindow = (year, today = new Date()) => {

    const closing = new Date(year, 11, 31);
    const to = year === today.getFullYear() && today < closing ? today : closing;

    return { days: Math.round((to - new Date(year, 0, 1)) / 86400000) + 1, before: dateToString(to) };
};

// One label per month, above the first column that month begins in. A month the
// year has not reached yet is left unlabelled.
...
// of the other written-on days it outwrites, so the colour spreads over the
// lengths actually present. A few very long days would otherwise flatten
// everything else into the same faint green. The shade is the share of the
// mixture that is green, so the quietest day still shows and the busiest is
// full strength.
